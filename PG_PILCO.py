...
class PolicyGradient(object):
	...
	def _build_net(self):
		with tf.name_scope("inputs"):
			...
			self.tf_acts = tf.placeholder(tf.int32, [None, ], name="actions_num")  # for cartpole-v0, dim of action is 1. to change for other task.
		...
		with tf.name_scope('loss'):
			...
			loss = tf.reduce_mean(neg_log_prob * self.tf_vt)  # we are going to replace tf_vt with reward calculate with PILCO
	...
	def choose_action(self, observation):
		...
		action = np.random.choice(range(prob_weights.shape[1]), p=prob_weights.ravel())   # p: proba distribution # ravel()：如果没有必要，不会产生源数据的副本 flatten()则会产生源数据副本
		return action
	...
	def _discount_and_norm_rewards(self):
		...
		for t in reversed(range(len(self.ep_rs))):
			...
			discounted_ep_rs[t] = running_add      # len(discounted_ep_rs) = total step in an episode

		# we can't do the normalization
