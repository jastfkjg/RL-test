...
class PILCO:
    ...
    def optimize_controller_without_state(self, env, horizon, total_epoch=20, num_collect=10, gamma=1.):
        """optimize controller wih init state sample from env.observation_space"""
        s_x = np.diag(np.ones(self.state_dim) * 0.01)
        epoch = 0
        while epoch < total_epoch:
        # for epoch in range(total_epoch):
            # m_x = env.observation_space.sample()
            qpos = env.init_qpos + env.np_random.uniform(size=env.model.nq, low=-1., high=1.)
            qvel = env.init_qvel + env.np_random.uniform(size=env.model.nv, low=-5., high=5.)
            m_x = np.hstack((qpos, qvel))
            if self.reward.compute_reward(m_x) == 0.:
                continue
            m_x = m_x.astype("float64")
            m_x = np.expand_dims(m_x, 0)
            self.get_data(m_x, s_x, horizon, num_collect=num_collect, gamma=gamma)
            epoch += 1
            self.controller.optimize()
        print("Finish %d times optimization." %total_epoch)
    ...
    def get_data(self, m_x_init, s_x_init, horizon, num_collect=10, gamma=1.0, ac_sample_num=1,\
        ...
        for i in range(num_collect):
            # print("the %i th choice of action, total: %i" %(i, num_collect))
            total_reward = 0.

            random_num = random.random()
            if random_num < 0.8:
                m_u, s_u = self.controller.compute_action(m_x_init)
                if i == 0:
                    print("s_u: ", s_u)
                m_u = self.controller.sample_action(m_u, s_u)
                s_u = np.ones(self.control_dim) * 0.01
            else:
                # print("random action")
                m_u, s_u = self.controller.random_action()
            # print('mean and variance of action distribution: ', m_u, s_u)

            ep_m_x.append(np.squeeze(m_x_init, 0))
            # s_u is very small
            ep_ac.append(m_u)
            c_xu = self.controller.compute_covariance(m_x_init, s_x_init, m_u, s_u)
            m_x, s_x = self.propagate(m_x_init, s_x_init, m_u, np.diag(s_u), c_xu)
            ...
            for j in range(horizon):
                ...
                if self.reward.with_action:
                    ...
                    current_reward, done = self.reward.compute_gaussian_reward(np.squeeze(m_x, 0), \
                            s_x, threshold=reward_threshold)
                
                total_reward += (current_reward * (gamma ** (j + 1)))
                # print(m_u, s_u, current_reward)
                
                if done or j == horizon - 1:
                    ep_reward.append(total_reward)
                    # print("the %i th choice, total reward:  %f " %(i, total_reward))
                    # print(s_u)
                    break            
