...
class PendulumReward(Reward):
    ...
    def compute_gaussian_reward(self, m_state, s_state, m_action, sample_num=40):
...
class InvertedPendulumReward(Reward):
    ...
    def compute_gaussian_reward(self, m_state, s_state, sample_num=40, threshold=0.5):
        with tf.Graph().as_default() as graph:
            with tf.Session(graph=graph) as sess:
                ...
                try:
                    ...
                    dist_obs = DiagGaussian(m_state, abs(np.diag(s_state)))
                    states = []
                    for i in range(sample_num):
                        state = dist_obs.sample()   # [sample_num, state_dim]
                        states.append(state)
                    # with tf.Session() as sess:
                    states = sess.run(states)
